"""
kvmem/hmn_adaptive_trainer.py — fork of kvmem/hmn.py (cloned from the
2026-07-27 `hmn_v3_backup.py` snapshot) adding ADAPTIVE weave_mix reweighting:
the `weave_mix` branch of train() only (chain_steps/stitch_mix/default
traj_mix branches are untouched, byte-identical to hmn.py) tracks each
trajectory's own difficulty and, at every eval step, re-derives that
trajectory's SAMPLING WEIGHT from it — training effort automatically
shifts toward whichever shape(s) are lagging instead of staying fixed at
whatever the config authored up front.

Motivated by hmn_weave_c64: with 3 shapes (batch/stream/interleave_delayed)
mixed at equal weight/B, `stream` reached 42.3% match while `batch`/
`interleave_delayed` sat at 9.5% — the fixed uniform mix has no way to
notice that split and lean into it.

repeat_batch adaptation is DISABLED — kept simple, one feedback signal
(weight) at a time rather than two coupled ones. Each trajectory's
`repeat_batch` stays fixed at whatever the config/DSL `B<n>` token set
(`base_repeat_batch`); `_adapt_reweight`'s repeat_batch-scaling line is
commented out, not deleted, in case it's worth reviving later once
weight-only adaptation is validated on its own.

New hp flags (only read inside the weave_mix branch; absent/False on every
existing config, so those are unaffected if run through this file):
  adaptive          bool,  default False — turns the mechanism on at all
  adapt_signal      'val_match' (default) | 'train_loss' — which per-
                    trajectory signal drives reweighting. val_match only
                    updates at eval steps (the actual target metric, but
                    coarse-grained); train_loss updates an EMA every step
                    (finer-grained, noisier, and available before the
                    first eval)
  adapt_repeat_max  int, default 16 — UNUSED while repeat_batch adaptation
                    is disabled; kept only so re-enabling that code later
                    doesn't need a new flag
  adapt_ema_alpha   float, default 0.9 — train_loss EMA smoothing
  adapt_floor       float, default 0.05 — minimum relative weight share
                    even for an already-solved trajectory, so nothing gets
                    starved to literal zero coverage
  adapt_temp        float, default 1.0 — softmax TEMPERATURE over normalized
                    difficulty (`_temp_softmax_rescale`). LOWER is more
                    aggressive (T->0 approaches one-hot on the single
                    hardest trajectory); HIGHER approaches uniform (T->inf
                    gives every trajectory equal weight regardless of
                    difficulty). At the loss spreads actually observed so
                    far, T=1.0 barely moves weights off ~1/3 each — T in
                    the 0.2-0.3 range is closer to a visibly aggressive
                    reallocation

Mechanism (`_adapt_reweight`, weave_mix branch): per-trajectory difficulty
is `100 - last_match` (val_match) or `ema_loss` (train_loss), normalized to
the CURRENT mix's own mean (1.0 = mix-average difficulty), then passed
through `_temp_softmax_rescale` — a softmax at `adapt_temp` over those
normalized values, rescaled so a perfectly uniform difficulty still maps
every trajectory back to exactly 1.0 (same convention the rest of the
formula assumes). A trajectory exactly at the mix's average difficulty
keeps its base_weight unchanged regardless of `adapt_temp` — this is
relative reweighting within the mix, not an absolute threshold. Harder-
than-average trajectories get scaled up, easier ones scaled down but never
below `adapt_floor`'s relative share. Logged every adaptation via the same
`weave_mix=[...]` table format as the static path, tagged `(adapted)` — the
`B<n>` shown there is just each trajectory's fixed base_repeat_batch, since
repeat_batch adaptation is disabled.

**LR schedule (weave_mix branch only): fixed after linear warmup, NOT
cosine** — deliberately different from hmn.py's weave_mix branch. A
decaying schedule assumes the training signal gets easier to fit over time
and anneals toward it; here the signal itself keeps shifting (that's the
whole point of adaptive reweighting), so annealing lr toward ~0 late in a
stage would blunt the model's ability to respond to a newly up-weighted
hard trajectory. `cosine_T0`/`cosine_T_mult`/`lr_schedule`/`lr_min` in a
config are read by every OTHER branch (chain_steps/stitch_mix/default
traj_mix, unchanged from hmn.py) but silently ignored by this one.

See docs/HMN_RECIPE.md for a from-scratch walkthrough and docs/HISTORY.md
for design rationale and prior-mechanism history (both describe hmn.py; this
file only adds the adaptive layer on top, described above).
"""
        ...
        if 'weave_mix' in stage:
            ...
            for wcfg in weave_mix_cfg:
                ...
                trajectories.append(dict(weight=wcfg['weight'], pattern=pname, n_chunks=w_n_chunks,
                                         ...
                                         base_weight=wcfg['weight'], base_repeat_batch=w_repeat_batch,
                                         ema_loss=None, last_match=None))
            traj_weights = np.array([t['weight'] for t in trajectories], dtype=np.float64)
            traj_weights = traj_weights / traj_weights.sum()

            # Adaptive reweighting (this file's whole point — see module docstring):
            # every eval step, re-derive each trajectory's sampling weight AND
            # repeat_batch from how much it's currently struggling, so training
            # effort automatically shifts toward whichever shape(s) are lagging
            # instead of staying fixed at whatever the config authored up front.
            ...
            adapt_repeat_max = hp.get('adapt_repeat_max', 16)  # clip — project history shows unbounded B backfires
            adapt_ema_alpha  = hp.get('adapt_ema_alpha', 0.9)  # train_loss EMA smoothing
            adapt_floor      = hp.get('adapt_floor', 0.05)  # min relative pull even for an already-solved trajectory
            adapt_temp       = hp.get('adapt_temp', 1.0)  # softmax temperature over normalized difficulty — LOWER
                                                           # is more aggressive (T->0 approaches one-hot on the
                                                           # single hardest trajectory), HIGHER approaches uniform
                                                           # (T->inf gives every trajectory equal weight regardless
                                                           # of difficulty). T=1.0 is a mild default; the loss
                                                           # spreads actually observed so far barely moved weights
                                                           # off ~1/3 each at T=1.0 — something like T=0.2-0.3 is
                                                           # needed for a visibly aggressive reallocation
            ...
            def _temp_softmax_rescale(diffs):
                """softmax(diffs/adapt_temp), rescaled so a perfectly uniform
                difficulty (all trajectories equally hard) maps back to d=1.0 for
                EACH trajectory — preserves the same convention the old power-based
                scaling used, so the floor-blending formula below doesn't change.
                Lower adapt_temp => peakier softmax => more aggressive reallocation."""
                n = len(diffs)
            ...
            def _adapt_reweight():
                """Recompute traj_weights (sampling probability only) from the chosen
                signal. difficulty is normalized to the mix's own mean so a trajectory
                exactly at the mix's average difficulty gets its base weight back
                unchanged; harder-than-average trajectories get scaled up, easier
                ones scaled down (but never below adapt_floor's relative share).

                repeat_batch adaptation is DISABLED (kept simple, per-trajectory
                repeat_batch stays fixed at whatever the config/DSL set — see
                base_repeat_batch) — weighting alone is the whole feedback signal
                for now. The scaling code is left commented rather than deleted in
                case it's worth reviving once weight-only adaptation is validated."""
                ...
                for t, d in zip(trajectories, diffs):
                    ...
                    # t['repeat_batch'] = int(np.clip(round(t['base_repeat_batch'] * d), 1, adapt_repeat_max))
                new_weights = np.array([t['weight'] for t in trajectories], dtype=np.float64)
                return new_weights / new_weights.sum()

            _log_weave_mix('')

            # Fixed LR after linear warmup — deliberately NOT cosine, unlike hmn.py's
            # weave_mix branch. A decaying schedule assumes the training signal gets
            # easier to fit over time and anneals accordingly; here the SIGNAL ITSELF
            # keeps shifting (adaptive reweighting continually moves effort onto
            # whichever trajectory is currently hardest), so annealing lr toward ~0
            # late in a stage would blunt the model's ability to actually respond to
            # a newly up-weighted hard trajectory. Any `cosine_T0`/`cosine_T_mult`/
            # `lr_schedule`/`lr_min` in the config are ignored by this branch.
            def _lr(s):
                if s <= warmup_steps:
            ...
            stage_best_val = -1.0
            _eval_count = 0  # eval_count>=2 required before val_match-driven adaptation kicks in — see below
            ...
            for local_step in pbar:
                ...
                _ema_str = '[' + ','.join(f'{t["ema_loss"]:.2f}' if t['ema_loss'] is not None else 'NA'
                                          for t in trajectories) + ']'
                # Live preview of what _adapt_reweight would currently produce from
                # train_loss EMAs, RECOMPUTED FRESH EVERY STEP for display only — does
                # NOT mutate traj['weight']/traj_weights (those only actually change at
                # eval steps, see _adapt_reweight itself). Lets you watch the mechanism
                # trend between evals instead of only seeing a jump at each eval.
                _known = [t['ema_loss'] for t in trajectories if t['ema_loss'] is not None]
                if _known:
                    _fallback = sum(_known) / len(_known)
                    _live_diffs = np.array([t['ema_loss'] if t['ema_loss'] is not None else _fallback
                                            for t in trajectories])
                    _live_diffs = _live_diffs / max(_live_diffs.mean(), 1e-8)
                    _live_diffs = _temp_softmax_rescale(_live_diffs)
                    _live_w = np.array([t['base_weight'] * (adapt_floor + (1 - adapt_floor) * d)
                                        for t, d in zip(trajectories, _live_diffs)])
                    _live_w = _live_w / _live_w.sum()
                    _w_str = '[' + ','.join(f'{w:.2f}' for w in _live_w) + ']'
                else:
                    _w_str = 'NA'
                pbar.set_postfix(loss=f'{loss_f:.3f}', lr=f'{lr:.1e}', ema=_ema_str, w=_w_str, refresh=False)
                if local_step % log_every == 0:
                    _jlog(dict(step=global_step, loss=round(loss_f, 5), lr=lr,
                               ema_loss=[round(t['ema_loss'], 4) if t['ema_loss'] is not None else None
                                        for t in trajectories]))
                ...
                if local_step % stage_eval_every == 0 or local_step == n_steps:
                    ...
                    if adaptive:
                        # val_match's very first reading is the noisiest possible
                        # signal (least-trained model) — skip adapting on it and
                        # wait for the second eval so at least one real trend point
                        # exists. train_loss doesn't have this problem (its EMA has
                        # already been accumulating every step since step 1).
