"""
`hmn_tpu_recall1024_jax_curriculum.py` (renamed from `..._flat_rope_jax_
curriculum.py` — no longer accurately "flat" once the weave_mix stopped
being a single 16-chunk target) — same architecture as
`hmn_tpu_recall1024_flat_rope_jax_smallbatch.py` (`d=128/n_layers=16/
...
`adaptive=True` (NEW, 2026-07-31) — without this, uniformly weighting 60
entries of wildly different difficulty (128-byte recall vs 1024-byte
suffix recall) means the model never gets pushed toward whichever entries
it's actually struggling with; sampling stays static at the config weight
forever. JAX port of kvmem.hmn's own `_adapt_reweight` (identical formula,
see `kvmem/hmn_jax.py`'s `train_jax`) — `adapt_signal='val_match'` (default,
matches kvmem.hmn's own default): after the 2nd eval onward, sampling
weight shifts toward entries with the LOWEST match%, scaled by
`adapt_floor + (1-adapt_floor)*softmax(normalized_difficulty)`, so no
entry (even one already at 100%) drops below `adapt_floor`'s relative
share — verified the rescaling formula directly (uniform difficulty stays
uniform, a harder entry gets upweighted, easier ones scale down, sum
preserved) before deploying.

Run (never two jobs at once):
    python3 -m kvmem.hmn_jax --config kvmem/configs/hmn_tpu_recall1024_jax_curriculum.py
"""

from kvmem.hmn_jax import load_config

hp = load_config('kvmem/configs/hmn_tpu_recall1024_jax_smallbatch.py')
hp['name'] = 'hmn_tpu_recall1024_jax_curriculum'
