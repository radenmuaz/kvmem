...
3. **chat-tags** (`experiments/chat_tags/`, isolated from `kvmem/`) — explicit boundary tokens wrapping the `iq_global_rw` layout. Phase A passed. Phase B confounded (60.2%). Phase B2 (traj_mix fix) oscillating, 70.8% peak. Phase B3 (single clean decay) **CONVERGED**: 75.9%, Win A 100%, Win B 100%, Win C stuck 27.8-30.6% — diagnosed root cause: Win C's IR turns were *degrading* quality (destructive interference from 3 windows sharing one `<query>` addressing key), not a SLOT-capacity ceiling. **Phase B4 (window-specific `<query_a/b/c>` tags) CONVERGED — 94.9%: Win A 100%, Win B 100%, Win C 84.7%** (stable across final 4 checkpoints, ~3× B3's Win C, vs untagged baseline 77.8%/100%/77.8%/55.6%). Confirms addressing (not capacity/rank) was the real bottleneck — see `docs/SRS_RECIPE.md § Fast-Weight Rank and Addressing` for the theory. Falls short of the 90%-all-windows target only on Win C (close, converged, not oscillating). **DenseNet-style depth-wise cross-layer SLOT-KV concatenation built and run** (`experiments/densenet_kv/`) — architecture verified correct (cross-layer growth confirmed numerically, causality confirmed leak-free), but the ablation against B4 was **inconclusive**: B4 was warm-started from B3's ~348k-step-deep checkpoint while densenet_kv trained from scratch in the same 80k-step budget and was still rising (1.9%→20.8%, never converged) when the budget ran out — not a fair speed comparison. Correct follow-up (not yet built): a from-scratch **standard**-architecture control at the same 80k budget to establish the fair baseline. See `docs/FEEDBACK_RESULTS.md § Chat-tags experiment` / `§ DenseNet-KV ablation` and the plan at `/Users/muaz/.claude/plans/design-experiment-which-use-atomic-kay.md`.
