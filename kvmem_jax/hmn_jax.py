...
def _build_trajectory(hp: dict, entry: dict, stage_chunk_len: int, enc_hops: int | None = None) -> dict:
    """One weave_mix entry -> {pos_content, mask (jnp), L, chunk_len, weight}.

    `enc_hops` (default None -> falls back to `hp.get('enc_hops', -1)`,
    unchanged legacy behavior) opts into the bounded encoding-chain window
    (see `chunk_mask_fb_traj`'s own docstring) — passed explicitly by
    `train_jax`'s stage loop as `stage.get('enc_hops', hp.get('enc_hops',
    -1))` so `enc_hops` is STAGE-scoped, matching `hop_drop_prob`'s own
    per-stage override (a real gap fixed 2026-07-31: different curriculum
    stages legitimately want different `enc_hops` — e.g. unbounded for an
    early n_chunks-scaling stage, then a bounded window only once the
    target shape is reached — and the global-only version silently ignored
    any stage-level `enc_hops` a config tried to set). When enabled, `mask`
...
def ar_decode_traj_kv(model, chunks_arr, state_len: int, state_vocab_size: int,
    ...
    # up to rb['c0']) in ONE dense forward pass, capturing the KV cache AND
    # the prefix's own final logit — that logit is the model's prediction for
    # the FIRST response token (position rb['c0']) and MUST be used to seed
    # it. A real bug, found 2026-07-31 via direct comparison against
    # `ar_decode_traj_nokv` on a real checkpoint (100% match there vs. 0-40%
    # here): the old code discarded this logit (`_, kv_cache = model(...)`)
    # and instead fed `tok[prefix_end]` — still its np.zeros(...) init value,
    # never written by anything before this point — into the model as if it
    # were a real known token, permanently corrupting that position's cached
    # key/value (every later step attends back to it, cascading the error
    # through the rest of decode). Fixed by using the prefix pass's own
    # logit to set the first response token BEFORE the per-step loop, then
    # shifting the loop to its natural "encode token_j, predict token_{j+1}"
    # form (out_len-1 iterations, not out_len — the first prediction came
    # from the prefix pass, not the loop).
    prefix_end = rb['c0']
    out_len = rb['out_len']
    prefix_tok = jnp.asarray(tok[:prefix_end], dtype=jnp.int32)[None, :]
    prefix_mask = jnp.asarray(mask_np[:prefix_end, :prefix_end])
    prefix_logits, kv_cache = model(prefix_tok, prefix_mask, return_kv=True)
    L_cached = prefix_end
    if out_len > 0:
        tok[prefix_end] = int(jnp.argmax(prefix_logits[0, -1]))

    for j in range(out_len - 1):
        pos = prefix_end + j
        # Single new token (the just-predicted one), attending to everything
        # cached so far (+1 for itself once appended) — matches kvmem.hmn's
        # own KV-cache decode mask slicing convention
        # (`full_mask[pos:pos+1, :pos+1]`).
        ...
        tok[pos + 1] = int(jnp.argmax(logits[0, -1]))
...
def _make_decode_jit_body(prefix_end: int, out_len: int):
    ...
    n_iters = max(out_len - 1, 0)  # the first response token comes from the prefix pass's
                                    # own final logit (set into tok_buf0 by the caller BEFORE
                                    # this function is invoked), not from this loop — see
                                    # ar_decode_traj_kv_jit's own docstring/comment for the
                                    # 2026-07-31 bug this fixes (mirrors ar_decode_traj_kv's
                                    # own eager fix exactly)
    ...
    def decode_jit(model, tok_buf0, kv_buf0, step_masks):
        def body(j, carry):
            ...
            pos = prefix_end + j
            # Encode the token AT `pos` (already known — either the prefix-pass
            # prediction, for j==0, or this loop's own previous iteration's
            # prediction) to get the logit that predicts position pos+1.
            ...
            tok_buf = jax.lax.dynamic_update_slice_in_dim(tok_buf, next_tok[None], pos + 1, axis=0)
            return (tok_buf, kv_new)
        tok_buf, _ = jax.lax.fori_loop(0, n_iters, body, (tok_buf0, kv_buf0))
        return tok_buf
    return decode_jit
...
def ar_decode_traj_kv_jit(model, chunks_arr, state_len: int, state_vocab_size: int,
    ...
    prefix_tok = jnp.asarray(tok[:prefix_end], dtype=jnp.int32)[None, :]
    prefix_mask = jnp.asarray(mask_np[:prefix_end, :prefix_end])
    prefix_logits, kv_prefix = model(prefix_tok, prefix_mask, return_kv=True)
    kv_buf0 = list((jnp.pad(k, ((0, 0), (0, 0), (0, out_len), (0, 0))),
                   jnp.pad(v, ((0, 0), (0, 0), (0, out_len), (0, 0))))
                  for k, v in kv_prefix)
    # The prefix pass's own final logit is the model's prediction for the FIRST
    # response token — must seed tok[prefix_end] with it before building tok_buf0
    # (see `_make_decode_jit_body`'s own comment for the 2026-07-31 bug this
    # fixes: the old code discarded this logit and fed tok[prefix_end]'s np.zeros
    # init value into the model as if it were a real token, permanently
    # corrupting that position's cached KV entry).
    if out_len > 0:
        tok[prefix_end] = int(jnp.argmax(prefix_logits[0, -1]))
    ...
    match_pct = 100.0 * float(np.sum(rb_gen[:len(rb_target)] == rb_target)) / max(len(rb_target), 1)
    if return_bytes:
        return dict(match_pct=match_pct, warmup=warmup_src, target=rb_target, generated=rb_gen)
    return dict(match_pct=match_pct)


def _fmt_bytes(byte_arr) -> str:
    """Printable-safe repr: printable ASCII as-is, else \\xNN. Ported from
    kvmem/eval_qualitative.py's own `_fmt`."""
    return ''.join(chr(int(b)) if 0x20 <= int(b) < 0x7f else f'\\x{int(b):02x}' for b in byte_arr)


def log_qualitative_eyeball(model, trajectories: list[dict], hp: dict, _log,
                            seq_name: str = 'up_counter', max_entries: int = 8) -> None:
    """JAX counterpart to `kvmem/eval_qualitative.py` (torch, standalone
    script) — restricted to this file's own single-non-refine-Q-per-entry
    scope (no `batch`/`stream`/`interleave_delayed` multi-op support needed,
    since `train_jax` never trains those shapes), and — unlike the torch
    version — called INLINE from `train_jax` itself (after every stage
    completes, and once more at the very end of training) rather than as a
    separate manually-invoked script, per explicit request to make this a
    standing part of the training loop's own output, not an extra step to
    remember to run. Prints warmup/target/generated/diff side-by-side per
    entry (up to `max_entries`, since a full weave_mix can have dozens) —
    the same qualitative distinction match% alone can't show (e.g.
    "degrades into plausible-but-wrong bytes" vs. "collapses into pure
    non-printable noise" — see CLAUDE.md's `weave_mix_accum_rnn` entry for
    why this distinction mattered there)."""
    _log(f'\n=== qualitative eyeball (seq={seq_name}) ===')
    for t in trajectories[:max_entries]:
        val_seqs = make_test_sequences(t['n_chunks'] * t['chunk_len'])
        if seq_name not in val_seqs:
            continue
        ...
        _log(f'  warmup (GT) : {_fmt_bytes(warmup)}')
        _log(f'  target (GT) : {_fmt_bytes(target)}')
        _log(f'  generated   : {_fmt_bytes(gen)}')
        _log(f'  diff        : {diff}')
...
def train_jax(hp: dict, log_base: str = 'logs'):
    ...
    # Data parallelism across all visible devices (e.g. a v6e-4 node's 4 chips) --
    # opt-in via hp['data_parallel'] (default False, every existing single-chip
    # config unaffected). Uses JAX's modern GSPMD auto-sharding (jax.jit + a Mesh +
    # NamedSharding on the batch inputs) rather than jax.pmap or a manually-written
    # all-reduce -- verified locally (--xla_force_host_platform_device_count=4) that
    # this requires ZERO changes to the nnx.jit-decorated step functions themselves:
    # sharding only the batch/mask inputs along a 'data' mesh axis (model/optimizer
    # left unsharded/implicitly-replicated) reproduces the single-device loss and
    # resulting weights to float32 noise (~7e-9 max diff), because XLA's SPMD
    # partitioner inserts the necessary collective (e.g. the implicit all-reduce
    # inside `jnp.mean(nll)`) automatically from the input shardings -- no explicit
    # jax.lax.pmean call needed anywhere in the loss function. Per-device batch size
    # must be a whole number, i.e. B (or B_bucket) divisible by len(jax.devices()).
    data_parallel = hp.get('data_parallel', False)
    _mesh = None
    if data_parallel:
        _dp_devices = jax.devices()
        assert len(_dp_devices) > 1, f'data_parallel=True but only {len(_dp_devices)} device(s) visible'
        _mesh = Mesh(np.array(_dp_devices), axis_names=('data',))
        _log(f'[data_parallel] enabled — mesh over {len(_dp_devices)} devices: {_dp_devices}')

    def _shard_batch(x):
        """Shard a [B, ...] array along the batch axis across the data-parallel
        mesh; a no-op (plain jnp.asarray) when data_parallel is off."""
        if _mesh is None:
            return jnp.asarray(x)
        spec = P('data', *((None,) * (x.ndim - 1)))
        return jax.device_put(np.asarray(x), NamedSharding(_mesh, spec))

    def _replicate(x):
        """Place a non-batch array (e.g. a trajectory's shared [L,L] mask) as
        fully replicated across the data-parallel mesh; a no-op when off."""
        if _mesh is None:
            return x
        return jax.device_put(x, NamedSharding(_mesh, P()))

    def _log_tpu_util(tag: str):
        """Samples TPU utilization in-code — no manual `tpu-info` SSH session
        needed. Two sources, both best-effort (swallow any failure rather
        than ever interrupting training over a monitoring call):
        (1) HBM usage via JAX's own `memory_stats()` — always available,
        this process's own device handle, no RPC.
        (2) `tpu_info.metrics.get_chip_usage()` — the same Python API the
        `tpu-info` CLI itself is built on (`pip install tpu-info`, already
        on every TPU VM via `setup_tpu_jax.sh`), returning `Usage(device_id,
        memory_usage, total_memory, duty_cycle_pct)` per chip via a local
        gRPC call to libtpu's own metrics server (port 8431) — confirmed
        empirically this ONLY responds once a framework (this training
        process itself) is actively attached to the TPU; querying it with
        nothing running returns `UNAVAILABLE`/connection-refused, which is
        why this can't be sampled from a separate one-off script the way
        HBM stats can — it must run INSIDE the live training process, which
        is exactly what this call does. `duty_cycle_pct` is the actual
        TensorCore utilization number `tpu-info`'s CLI table prints.
        Called at stage start (post-compile) and periodically during
        training so the log carries REAL per-stage samples across several
        batches, not just one snapshot."""
        try:
            if jax.default_backend() == 'tpu':
                dev = jax.local_devices()[0]
                stats = dev.memory_stats()
                if stats:
                    used_gib = stats.get('bytes_in_use', 0) / 2**30
                    limit_gib = stats.get('bytes_limit', 0) / 2**30
                    _log(f'  [util:{tag}] HBM(jax): {used_gib:.2f}/{limit_gib:.2f} GiB '
                        f'({100*used_gib/max(limit_gib, 1e-9):.1f}%)')
        except Exception as e:
            _log(f'  [util:{tag}] HBM(jax) query failed: {e}')
        try:
            from tpu_info import metrics as _tpu_metrics, device as _tpu_device
            # get_local_chips() returns a single (chip_type, count) tuple directly
            # (NOT a list of tuples) -- a prior version of this code did
            # `chip_type, _count = chips[0]`, which indexes into the tuple to get
            # just the TpuChip object and then tries to unpack THAT as a pair,
            # producing "cannot unpack non-iterable TpuChip object". Fixed by
            # unpacking the tuple itself, no [0] indexing.
            chip_type, _count = _tpu_device.get_local_chips()
            if chip_type is not None:
                usages = _tpu_metrics.get_chip_usage(chip_type)
                for u in usages:
                    _log(f'  [util:{tag}] chip{u.device_id}: HBM={u.memory_usage/2**30:.2f}/'
                        f'{u.total_memory/2**30:.2f}GiB  duty_cycle={u.duty_cycle_pct:.1f}%')
        except Exception as e:
            _log(f'  [util:{tag}] tpu_info query failed: {e}')
    ...
    for stage_i, stage in enumerate(hp['curriculum']):
        ...
        stage_enc_hops = stage.get('enc_hops', hp.get('enc_hops', -1))
        trajectories = [_build_trajectory(hp, e, stage['chunk_len'], enc_hops=stage_enc_hops)
                        for e in stage['weave_mix']]
        ...
        B = stage['B']
        if data_parallel:
            assert B % len(jax.devices()) == 0, \
                f'data_parallel=True requires B ({B}) divisible by device count ({len(jax.devices())})'
        ...
        if data_parallel:
            ...
            # mesh -- 'mask' always exists, 'mask_bucket'/'loss_mask' only in the
            # bucketed/segmented-forward branches above. Model/optimizer params are
            # deliberately left unsharded/implicit -- verified locally that XLA's SPMD
            # partitioner replicates them automatically to match the sharded batch.
            for t in trajectories:
                t['mask'] = _replicate(t['mask'])
                if 'mask_bucket' in t:
                    t['mask_bucket'] = _replicate(t['mask_bucket'])
                if 'loss_mask' in t:
                    t['loss_mask'] = _replicate(t['loss_mask'])
        ...
        stage_best_val = -1.0
        t_start = time.time()
        _util_samples_taken = 0  # cap a few in-training samples per stage (not every
                                  # log_every tick over a 100000-step stage) — "several
                                  # batches" worth, per explicit request, not a firehose
        ...
        for local_step in pbar:
            ...
            if data_parallel:
                assert B_eff % len(jax.devices()) == 0, \
                    f'data_parallel=True requires B_eff ({B_eff}) divisible by device count ({len(jax.devices())})'
                tokens = _shard_batch(tok_np.astype(np.int32))
            else:
            ...
            if is_first_call:
                ...
                if len(_compiled_step_fn_ids) == _n_distinct_shapes:
                    ...
                    _log_tpu_util(f'stage{stage_i}-post-compile')
            ...
            if local_step % log_every == 0 or local_step == n_steps:
                ...
                if _util_samples_taken < 3:
                    _log_tpu_util(f'stage{stage_i}-step{local_step}')
                    _util_samples_taken += 1
        ...
        log_qualitative_eyeball(model, trajectories, hp, _log)

    _log('\n=== training complete — final qualitative eyeball ===')
    log_qualitative_eyeball(model, trajectories, hp, _log)
