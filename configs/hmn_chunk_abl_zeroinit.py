"""
Architecture ablation: zero_init_residual=True (ReZero-style) vs baseline.
See hmn_chunk_abl_baseline.py for the fixed exp constant + rationale.

zero_init_residual: zero-init attn W_O and FFN W2 per block, so every
block starts as an identity map. Cheapest of the candidate fixes for the
random-init-IR instability — pure init change, no forward-pass cost.

Run:
    python -m kvmem.train_hmn_chunk \
        --config configs/hmn_chunk_abl_zeroinit.py --device mps
"""
...
hp = dict(
    ...
    chunk_attn=256,
    name='hmn_chunk_abl_zeroinit', seed=42,
