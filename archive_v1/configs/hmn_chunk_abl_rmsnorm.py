"""
Architecture ablation: rmsnorm=True (true RMSNorm) vs baseline (LayerNorm).
See hmn_chunk_abl_baseline.py for the fixed exp constant + rationale.

rmsnorm: replaces nn.LayerNorm (mean-centering + bias) with true RMSNorm
(scale-only, no mean-centering, no bias) at norm1/norm2/norm_out.

Run:
    python -m kvmem.train_hmn_chunk \
        --config configs/hmn_chunk_abl_rmsnorm.py --device mps
"""
...
hp = dict(
    ...
    chunk_attn=256,
    name='hmn_chunk_abl_rmsnorm', seed=42,
