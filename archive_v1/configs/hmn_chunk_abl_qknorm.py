"""
Architecture ablation: qk_norm=True vs baseline.
See hmn_chunk_abl_baseline.py for the fixed exp constant + rationale.

qk_norm: RMS-normalize Q/K (learned per-dim scale) before RoPE/dot-product.
Stabilizes attention logit scale — most directly targeted at the kind of
divergence seen in the original random-init-IR failure.

Run:
    python -m kvmem.train_hmn_chunk \
        --config configs/hmn_chunk_abl_qknorm.py --device mps
"""
...
hp = dict(
    ...
    chunk_attn=256,
    name='hmn_chunk_abl_qknorm', seed=42,
