...
**RESOLVED (2026-07-14): `dualattn_nc8_slot8_ir` completed 60000/60000 steps — RMSNorm did NOT fix window G.** Final val/test STITCHED_MEAN 38.9%/44.2% (down from a 48.6% val peak at step 35000; best checkpoint 51.4%). Window G (`span(6,8)`) ended at val 5.6%/test 8.3%, never breaking out of the 5-14% band it sat in from step 5000 onward — same qualitative failure as the earlier LayerNorm nc8 attempt (4.2%/25.0%). **Conclusion: window G is a structural problem (likely RoPE-distance or undertraining-specific to that window's position), not a normalization-choice problem.** Two bugs found and fixed before the clean run: (1) warm-start logic silently dropped growing tensors instead of prefix-copying; (2) MPS OOM crash from `chunk_attn` never being wired into `DualAttnModel` (fixed, `chunk_attn=256, B=3`). Full progress table and incident writeups in `docs/SRS_RECIPE.md`.
...
- `grad_checkpoint` implemented for `DualAttnModel` (two granularities), predicted net *slower* on this MPS hardware (checkpointing always adds recompute; the payoff requires batch-size headroom MPS doesn't have) — queued as an empirical speed test, not yet run (config ready: `experiments/attn_dual/configs/dualattn_nc8_gradckpt_speedtest.py`).

**Next in queue** (never two jobs at once): grad-checkpoint speed test (off/block/attn, cheap — 2000 steps each) → window-G oversampling fix (next lever now that RMSNorm is ruled out — restructure `traj_mix` to give window G more training weight, not attempted first since the RMSNorm test was cheaper to try first) → `juz1.txt` streaming-loop implementation (design complete, not yet built — see `docs/SRS_RECIPE.md § Concrete juz1.txt scaling design`) → chain-memory implementation (new architecture design, see `docs/SRS_RECIPE.md § Chain memory` — bounded, persistent, resumable `CHAIN_SLOT` state, a real departure from today's fully-independent-window design).
